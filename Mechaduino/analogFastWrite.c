...
#ifdef USE_10_BIT
static int _writeResolution = 10;  // edit changed from 8
#else
...
#ifdef USE_10_BIT
        value = mapResolution(value, _writeResolution,
                              10);  // change to 10 for 10 bit... must also change  TCx->COUNT8.PER.reg = 0x3FF
#else
