...
void dirInterrupt() {
    if (REG_PORT_IN0 & PORT_PA11)
        dir = false;  // check if dir_pin is HIGH
    else
        dir = true;
}
...
void calibrate() {  /// this is the calibration routine
    ...
    {
        SerialUSB.println("Wired backwards");  // rewiring either phase should fix this.  You may get a false message if
                                               // you happen to be near the point where the encoder rolls over...
        return;
    }
    ...
    // SerialUSB.println(" ");
    // SerialUSB.println("ticks:");                        //"ticks" represents the number of encoder counts between
    // successive steps... these should be around 82 for a 1.8 degree stepper SerialUSB.println(" ");
    for (int i = 0; i < spr; i++) {
...
float read_angle() {
    ...
    enableTCInterrupts();  // can't use readEncoder while in closed loop
    // delay(10);
...
void serialCheck() {  // Monitors serial for commands.  Must be called in routinely in loop for serial interface to
                      // work.
    ...
    if (SerialUSB.available()) {
        ...
        switch (inChar) {
            ...
            case 'a':  // anticogging
                // antiCoggingCal();
                use_anti_cogg = !use_anti_cogg;
                SerialUSB.println(use_anti_cogg);
                break;
                               ...
                               ///////////////////////////////
{
    ...
    if (angleTemp & (1 << 11))
        SerialUSB.println(
            "  MAGH - magnetic field strength too high, set if AGC = 0x00. This indicates the non-linearity error may "
            "be increased");

    if (angleTemp & (1 << 10))
        SerialUSB.println(
            "  MAGL - magnetic field strength too low, set if AGC = 0xFF. This indicates the output noise of the "
            "measured angle may be increased");
    ...
    if (angleTemp & (1 << 8))
        SerialUSB.println(
            "  LF - offset compensation completed. At power-up, an internal offset compensation procedure is started, "
            "and this bit is set when the procedure is completed");
...
void setupTCInterrupts() {  // configure the controller interrupt
    ...
    TC5->COUNT16.CC[0].reg = (int)(round(48000000 / FPID_FREQ));  // 0x3E72; //0x4AF0;
    WAIT_TC16_REGS_SYNC(TC5)
...
void enableExternalTCInterrupts(bool enable) {
    USE_EXTERNAL_INTERRUPTS = enable;
}

void antiCoggingCal() {  // This is still under development...  The idea is that we can calibrate out the stepper
                         // motor's detent torque by measuring the torque required to hold all possible positions.
...
void hybridControl() {  // still under development
    ...
    static float iLevel =
        0.6;  // hybrid stepping current level.  In this mode, this current is continuous (unlike closed loop mode). Be
              // very careful raising this value as you risk overheating the A4954 driver!
...
void serialMenu() {
    ...
    SerialUSB.println(
        " k  -  edit controller gains -- note, these edits are stored in volatile memory and will be reset if power is "
        "cycled");
...
void moveRel(float pos_final, int vel_max, int accel) {
    ...
    float pos = 0;
    float dpos =
        0.45;  // "step size" in degrees, smaller is smoother, but will limit max speed, keep below stepper step angle
    float vel   = 0;  //
    float vel_1 = 0;
...
void moveAbs(float pos_final, int vel_max, int accel) {
    ...
    float pos = r;
    float dpos =
        0.225;  // "step size" in degrees, smaller is smoother, but will limit max speed, keep below stepper step angle
    float vel   = 0;  //
    float vel_1 = 0;
    // int start = micros(); //for debugging
...
void move_to_zero() {
    disableTCInterrupts();
    float angle = readEncoder() / float(cpr) * 360.0;
    SerialUSB.println(angle);
    dir = angle > 180 ? true : false;
    while (angle > PA) {
        angle = readEncoder() / float(cpr) * 360.0;
        SerialUSB.println(angle);
        oneStep();
        delay(100);
    }
...
fixed_point_t float_to_fixed(double input) {
    return round(input * (1 << FIXED_POINT_FRACTIONAL_BITS));
}
